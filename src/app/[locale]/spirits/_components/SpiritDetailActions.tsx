...
import { deleteSpiritAction, updateSpiritAction } from '@/app/actions/spirits';
import type { Spirit, UpdateSpiritInput } from '@/modules/spirits/schema';
import { useLocalizedFields } from '@/hooks/useLocalizedFields';
import { toast } from '@/lib/toast';

interface SpiritDetailActionsProps {
  item: Spirit;
}

export function SpiritDetailActions({ item }: SpiritDetailActionsProps) {
  ...
  const handleUpdate = async (values: Record<string, unknown>) => {
    const result = await updateSpiritAction(item.id, values as UpdateSpiritInput);
    if (!result.success) {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      const result = await deleteSpiritAction(item.id);
      if (!result.success) {
